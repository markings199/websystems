<?php
	...
	include 'db.inc.php'; // Adjust the path to match your file structure
	$search_query = '';
	if (isset($_GET['search_query'])) {
		$search_query = $_GET['search_query'];
		$search_query = $conn->real_escape_string($search_query);
	
		$cars_sql = "SELECT * FROM cars WHERE car_type LIKE '%$search_query%' OR manufacturer LIKE '%$search_query%' OR model LIKE '%$search_query%'";
		$search_result = $conn->query($cars_sql);
	} else {
		$cars_sql = "SELECT * FROM cars"; // Default query to load all cars.
		$search_result = $conn->query($cars_sql);
	}
?>
<!DOCTYPE html>
<html lang="en" dir="ltr">
	...
	<body>
			...
			<header>
				...
				<nav>
					<ul>
						...
						<li class="to-my-thoughts">NEWS</li>
						<li class="to-gallary">GALLERY</li>
					</ul>
				</nav>
			</header>
		</section>
		...
		<section id="login" class="showcase">
			<div class="showcase-text" data-aos="slide-up">
				...
				<h1> Explore the Best Deals, Drive the Best Cars.</h1>
				<p>Quality Cars, Seamless Shopping</p>
			</div>
    ...
    ?>
			...
			<div class="search-car">
			<div class="search-car">
    <form class="search-car-form" action="index.php" method="get">
        <input class="search" type="text" name="search_query" placeholder="Search for cars...">
